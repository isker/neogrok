...
<section class="space-y-2 pt-2">
  <h2 class="text-2xl text-center">VCS history log</h2>
  <p>
    OpenGrok's VCS history log that was previously available at this URL is
    unfortunately not supported in neogrok.
  </p>
  {#if data.destinationUrl}
    <p>
      That being said, we've been able to produce a URL to view the
      file/directory on its home site, where you can hopefully get a history
      log. Head on over: <Link to={data.destinationUrl}
        >{data.destinationUrl}</Link
      >.
    </p>
  {:else if data.file}
    <p>
