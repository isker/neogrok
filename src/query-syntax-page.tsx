    ...
    <div className="space-y-2">
      ...
      <section className="space-y-2">
        ...
        <p>
          ...
          matches for the regex <Expression>/foo/</Expression> <em>or</em> the
          regex <Expression>/bar/</Expression>, but only in files that have
          matches for both <Expression>/foo/</Expression> <em>and</em>{" "}
          <Expression>/bar/</Expression>.
        </p>
        <p>
          First of all, if you wanted this query to behave as if it had only one
          expression, literally matching <Expression>/foo bar/</Expression>, you
          can either wrap it in double quotes, like{" "}
          <ExampleQuery query='"foo bar"' />, or by escaping the space, like{" "}
          <ExampleQuery query="foo\ bar" />; both of those queries are
          equivalent. (Single quotes have no special meaning in this regard.{" "}
          <ExampleQuery query="'foo bar'" /> contains two expressions, the first
          being <Expression>/&apos;foo/</Expression> and the second being{" "}
          <Expression>/bar&apos;/</Expression>.)
        </p>
        <p>
          As described above, expressions are <em>conjunctive</em> by default. A
          file must have at least one match for each expression to be included
          in the query results. Expressions may be made <em>disjunctive</em> by
          joining them with the special expression infix operator{" "}
          <code>or</code>. <ExampleQuery query="foo or bar" /> returns all
          matches for <Expression>/foo/</Expression> or{" "}
        ...
        <p>
          ...
          <ExampleQuery query="foo (bar or baz)" /> produces results from files
          with matches for both <Expression>/foo/</Expression> and at least one
          of <Expression>/bar/</Expression> or
      ...
      <section className="space-y-2">
        ...
        <p>
          ...
          special kinds of expressions can be created by prefixing them with
          certain flags, which are themselves composed of a few characters
          ending with a colon.
        </p>
        <p>
          ...
          contents (though the search engine tends to rank files containing file
          name matches higher than those containing only content matches, so you
          may have to scroll down or increase the number of files displayed to
          see the content matches for this query). If you only want the
          expression to match file names, use an expression prefixed with{" "}
          ...
          <tbody>
            ...
            <tr className="border">
              ...
              <td className="p-1">
                ...
                This is <em>not</em> a regex or a substring; each expression can
            ...
            <tr className="border">
              ...
              <td className="p-1">
                Unlike most other expressions, does not do anything on its own,
                but rather modifies other expressions in the query.{" "}
...
export { QuerySyntaxPage as Component };
